...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		// XXX Implement this method in week 2
		 ...
		 ArrayList<Integer> firstHopNeighbors = (ArrayList<Integer>) getNeighbors(v);
		 ArrayList<Integer> secondHopNeighbors = new ArrayList<Integer>();
		 
		 for(int i = 0; i < firstHopNeighbors.size(); i++){
			secondHopNeighbors.addAll(getNeighbors(firstHopNeighbors.get(i)));	 
		 }
	...
	private Integer[][] matrixMult(){
		
		Integer[][] result = new Integer[getNumVertices()-1][getNumVertices()-1];
		
		for(int i=0; i<getNumVertices(); i++){
			for(int j=0; j<getNumVertices(); j++){
				System.out.println(getNumVertices() + "+" + adjMatrix[i].length + " + " + getColumn(j).length); 
				result[i][j] = multiply(adjMatrix[i], getColumn(j)); 
			}
		}
	...
	private int[] getColumn(int i){
		...
		int[] column = new int[getNumVertices()-1];
		
		for(int v=0; v<column.length; v++){
			column[v] = adjMatrix[v][i];
		}
